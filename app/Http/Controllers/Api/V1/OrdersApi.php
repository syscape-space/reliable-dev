...
class OrdersApi extends Controller
{
    ...
    {
        if ($judger = Judger::query()->find(auth('api')->id()) and \request()->my){
            return successResponseJson(["data" => $judger->orders()->whereIn('order_status',['ongoing','done'])->with($this->arrWith())->orderBy("id", "desc")->paginate(15)]);
        }
