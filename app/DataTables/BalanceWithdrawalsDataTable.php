...
class BalanceWithdrawalsDataTable extends DataTable {
	...
	public function dataTable(DataTables $dataTables, $query) {
		return datatables($query)
			->addColumn('actions', 'admin.balancewithdrawals.buttons.actions')
			->addColumn('receipt_via', '{{ trans("admin.".$receipt_via) }}')
			->addColumn('transfer_picture', !empty($transfer_picture)?'<a href="{{ it()->url($transfer_picture) }}" target="_blank"><i class="fa fa-download fa-2x"></i></a>':'')
			->addColumn('withdrawal_status', '{{ trans("admin.".$withdrawal_status) }}')
			->addColumn('created_at', '{{ date("Y-m-d H:i:s",strtotime($created_at)) }}')	->addColumn('updated_at', '{{ date("Y-m-d H:i:s",strtotime($updated_at)) }}')	->addColumn('checkbox', '<div  class="icheck-danger">
                  <input type="checkbox" class="selected_data" name="selected_data[]" id="selectdata{{ $id }}" value="{{ $id }}" >
                  <label for="selectdata{{ $id }}"></label>
                </div>')
			->rawColumns(['checkbox', 'actions', "transfer_picture"]);
	}
	...
	public function query() {
		return BalanceWithdrawal::query()->with(['user_id'])->select("balance_withdrawals.*");
	...
	public function html() {
		...
		//->ajax('')
            ...
            ".filterElement('4', 'select', [
						...
						'bank_account'  => trans('admin.bank_account'),
						'system'        => trans('admin.system'),
					])."
            //withdrawal_statususer_id,amount,receipt_via,transfer_picture,withdrawal_status7
	...
	protected function getColumns() {
		return [
			...
			[
				'name'  => 'balance_withdrawals.receipt_via',
				'data'  => 'receipt_via',
				'title' => trans('admin.receipt_via'),
			],
			[
				'name'  => 'transfer_picture',
				'data'  => 'transfer_picture',
				'title' => trans('admin.transfer_picture'),
			],
			[
				'name'  => 'balance_withdrawals.withdrawal_status',
				'data'  => 'withdrawal_status',
				'title' => trans('admin.withdrawal_status'),
			],
			[
				'name'       => 'created_at',
				'data'       => 'created_at',
	...
	protected function filename() {
		return 'balancewithdrawals_'.time();
	}
