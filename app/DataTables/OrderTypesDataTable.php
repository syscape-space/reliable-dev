...
class OrderTypesDataTable extends DataTable {
	...
	public function dataTable(DataTables $dataTables, $query) {
		return datatables($query)
			->addColumn('actions', 'admin.ordertypes.buttons.actions')
			->addColumn('type_icon', '{!! view("admin.show_image",["image"=>$type_icon])->render() !!}')
			->addColumn('type_status', '{{ trans("admin.".$type_status) }}')
			->addColumn('created_at', '{{ date("Y-m-d H:i:s",strtotime($created_at)) }}')	->addColumn('updated_at', '{{ date("Y-m-d H:i:s",strtotime($updated_at)) }}')	->addColumn('checkbox', '<div  class="icheck-danger">
                  <input type="checkbox" class="selected_data" name="selected_data[]" id="selectdata{{ $id }}" value="{{ $id }}" >
                  <label for="selectdata{{ $id }}"></label>
                </div>')
			->rawColumns(['checkbox', 'actions', "type_icon"]);
	}
	...
	public function query() {
		return OrderType::query()->select("order_types.*");
	...
	protected function getColumns() {
		return [
			...
			[
				'name'  => l('type_name'),
				'data'  => l('type_name'),
				'title' => trans('admin.'.l('type_name')),
			],
			[
				'name'  => 'type_icon',
				'data'  => 'type_icon',
				'title' => trans('admin.type_icon'),
			],
			[
				'name'  => 'order_types.type_status',
				'data'  => 'type_status',
				'title' => trans('admin.type_status'),
			],
			[
				'name'       => 'created_at',
				'data'       => 'created_at',
