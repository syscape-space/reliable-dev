...
class UserExperiencesDataTable extends DataTable {
	...
	public function dataTable(DataTables $dataTables, $query) {
		return datatables($query)
			->addColumn('actions', 'admin.userexperiences.buttons.actions')
			->addColumn('experience_file', '{!! view("admin.show_image",["image"=>$experience_file])->render() !!}')
			->addColumn('created_at', '{{ date("Y-m-d H:i:s",strtotime($created_at)) }}')	->addColumn('updated_at', '{{ date("Y-m-d H:i:s",strtotime($updated_at)) }}')	->addColumn('checkbox', '<div  class="icheck-danger">
                  <input type="checkbox" class="selected_data" name="selected_data[]" id="selectdata{{ $id }}" value="{{ $id }}" >
                  <label for="selectdata{{ $id }}"></label>
                </div>')
			->rawColumns(['checkbox', 'actions', "experience_file"]);
	}
	...
	public function query() {
		return UserExperience::query()->with(['user_id',
				'user_job_id',
				'occupation_id',
				'specialtie_id'])->select("user_experiences.*");
	...
	protected function getColumns() {
		return [
			...
			[
				...
				'title' => trans('admin.experience_name'),
			],
			[
				'name'  => 'experience_file',
				'data'  => 'experience_file',
				'title' => trans('admin.experience_file'),
			],
			[
				'name'  => 'user_id.name',
				'data'  => 'user_id.name',
