                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" v-model="specialtie_id" name="specialtie_id"
                                    ...
                                    <option v-for="sp in specialties" :key="sp.id" :value="sp.id"
                                        :selected="sp.id == (qualification?.specialtie_id ?? '')">
...
import api from '../../utils/api';
    export default {
        props: ['user_id', 'qualification', 'specialties'],
        emits: ['refersh'],
        data() {
            return {
                errorMsg: null,
                ...
                specialtie_id: this.qualification?.specialtie_id ?? "0",
                qualification_name: this.qualification?.qualification_name,
                qualification_file: this.qualification?.qualification_file,
                comment: this.qualification?.comment,
        ...
        methods: {
            submitForm() {               
                ...
                if (this.specialtie_id != "0" && this.user_id && this.qualification_name) {
                    ...
                    api.post("/profile/qualification", qualificationData)
                    .then((response) => {
                        this.loading = false;
                        // this.editable = false;
                        this.errorMsg = null;
