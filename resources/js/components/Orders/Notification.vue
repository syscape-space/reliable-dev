<template>
    <section class="container mt-2">
      <div class="personal">
        <div class="personal-info">
          <div class="row w-100 mt-4 mx-0 px-0" >
           <div class="col-md-12">
            <h5>الإشعارات</h5>
          </div>
           <div class="col-md-12">
            <div class="d-flex align-iyems-center o_noti my-2">
              <div>
                <img class="ms-3" style="width: 40px; height: 40px; border-radius: 50%" :src="base_url + '/assets/assets/images/user.svg'" alt="">
              </div>
              <div class="flex-grow-1">
                <div class="btw-flex w-100">
                  <span style="color: #2b7b74">محمد مصطفي</span>
                  <span class=" ">
                    <img :src="base_url + '/assets/assets/images/o_cclo.svg'" alt="">
                  </span>
                </div>
                <p class="text-end my-1">
                  {{ $root._t("app.action") }}

                </p>
                <span style="color: #0995eb">
                  <img :src="base_url + '/assets/assets/images/prof-blue.svg'" alt="">
                  {{ $root._t("app.ago") }} 30 {{ $root._t("app.minute") }}</span>

              </div>
            </div>
            <div class="d-flex align-iyems-center o_noti my-2">
              <div>
                <img class="ms-3" style="width: 40px; height: 40px; border-radius: 50%" :src="base_url + '/assets/assets/images/user.svg'" alt="">
              </div>
              <div class="flex-grow-1">
                <div class="btw-flex w-100">
                  <span style="color: #2b7b74">محمد مصطفي</span>
                  <span class=" ">
                    <img :src="base_url + '/assets/assets/images/o_cclo.svg'" alt="">
                  </span>
                </div>
                <p class="text-end my-1">
                  {{ $root._t("app.action") }}

                </p>
                <span style="color: #0995eb">
                  <img :src="base_url + '/assets/assets/images/prof-blue.svg'" alt="">
                  {{ $root._t("app.ago") }} 30 {{ $root._t("app.minute") }}</span>

              </div>
            </div>
            <div class="d-flex align-iyems-center o_noti my-2">
              <div>
                <img class="ms-3" style="width: 40px; height: 40px; border-radius: 50%" :src="base_url + '/assets/assets/images/user.svg'" alt="">
              </div>
              <div class="flex-grow-1">
                <div class="btw-flex w-100">
                  <span style="color: #2b7b74">محمد مصطفي</span>
                  <span class=" ">
                    <img :src="base_url + '/assets/assets/images/o_cclo.svg'" alt="">
                  </span>
                </div>
                <p class="text-end my-1">
                  {{ $root._t("app.action") }}

                </p>
                <span style="color: #0995eb">
                  <img :src="base_url + '/assets/assets/images/prof-blue.svg'" alt="">
                  {{ $root._t("app.ago") }} 30 {{ $root._t("app.minute") }}</span>

              </div>
            </div>
            <div class="d-flex align-iyems-center o_noti my-2">
              <div>
                <img class="ms-3" style="width: 40px; height: 40px; border-radius: 50%" :src="base_url + '/assets/assets/images/user.svg'" alt="">
              </div>
              <div class="flex-grow-1">
                <div class="btw-flex w-100">
                  <span style="color: #2b7b74">محمد مصطفي</span>
                  <span class=" ">
                    <img :src="base_url + '/assets/assets/images/o_cclo.svg'" alt="">
                  </span>
                </div>
                <p class="text-end my-1">
                  {{ $root._t("app.action") }}

                </p>
                <span style="color: #0995eb">
                  <img :src="base_url + '/assets/assets/images/prof-blue.svg'" alt="">
                  {{ $root._t("app.ago") }} 30 {{ $root._t("app.minute") }}</span>

              </div>
            </div>
            <div class="d-flex align-iyems-center o_noti my-2">
              <div>
                <img class="ms-3" style="width: 40px; height: 40px; border-radius: 50%" :src="base_url + '/assets/assets/images/user.svg'" alt="">
              </div>
              <div class="flex-grow-1">
                <div class="btw-flex w-100">
                  <span style="color: #2b7b74">محمد مصطفي</span>
                  <span class=" ">
                    <img :src="base_url + '/assets/assets/images/o_cclo.svg'" alt="">
                  </span>
                </div>
                <p class="text-end my-1">
                  {{ $root._t("app.action") }}

                </p>
                <span style="color: #0995eb">
                  <img :src="base_url + '/assets/assets/images/prof-blue.svg'" alt="">
                  {{ $root._t("app.ago") }} 30 {{ $root._t("app.minute") }}</span>

              </div>
            </div>
            <div class="d-flex align-iyems-center o_noti my-2">
              <div>
                <img class="ms-3" style="width: 40px; height: 40px; border-radius: 50%" :src="base_url + '/assets/assets/images/user.svg'" alt="">
              </div>
              <div class="flex-grow-1">
                <div class="btw-flex w-100">
                  <span style="color: #2b7b74">محمد مصطفي</span>
                  <span class=" ">
                    <img :src="base_url + '/assets/assets/images/o_cclo.svg'" alt="">
                  </span>
                </div>
                <p class="text-end my-1">
                  {{ $root._t("app.action") }}

                </p>
                <span style="color: #0995eb">
                  <img :src="base_url + '/assets/assets/images/prof-blue.svg'" alt="">
                  {{ $root._t("app.ago") }} 30 {{ $root._t("app.minute") }}</span>

              </div>
            </div>
           </div>
            
          </div>
        </div>
      </div>
    </section>
</template>
<script>
export default {
  data(){
    return{
      base_url:base_url
    };
  },
}
</script>
<style scoped>

</style>